...
function removeEmptySpace(string){
    string=string.split(',').map(items=>items.split(''))
...
// Must have an even number of lowercase letters (do not count punctuation or spaces as letters)
    ...
    let lowerCaseCount= oddCharCodesRemoved.map(items=>items.filter((char) => char.match(/[a-z]/))).map(items=>items.length)
    let evenLowerCase=[]
...
// Must have at least 2 capital letters (honestly, I'm a little worried these requirements are too strict)
        ...
        let capitalsOnly=evenLowerCase.map(items=>items.filter((char) => char.match(/[A-Z]/)))
        let doubleCaps=[]
        ...
        let fin= finalArray.filter(items=>!items.includes("bark")).filter(items=>!items.includes("dog")).filter(items=>!items.includes("bone")).filter(items=>!items.includes("S")).filter(items=>items.length%5!==0)
